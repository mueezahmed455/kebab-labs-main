...
import { z } from 'zod'
import { ArrowLeft, ArrowRight, MapPin, CreditCard, Banknote, AlertCircle } from 'lucide-react'
...
function Field({
  ...
  return (
    <div>
      <label className="block text-brand-muted text-xs font-medium tracking-widest uppercase mb-1.5">
        {label}
      </label>
      {children}
      {error && (
...
function Input({ error, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { error?: string }) {
  return (
    <input
      {...props}
      className={cn(
        'w-full px-4 py-3 rounded-xl bg-brand-card border text-brand-white text-sm placeholder:text-brand-dim focus:outline-none transition-colors',
...
export default function CheckoutPage() {
  ...
  const { step, form, setStep, updateForm, setOrderReference } = useCheckout()
  ...
  const onStep1 = step1.handleSubmit((data) => {
    updateForm(data)
    setStep(2)
  })

  const onStep2 = step2.handleSubmit((data) => {
    updateForm(data)
    setStep(3)
  })

  const handlePlaceOrder = () => {
    const ref = `KBL-${crypto.randomUUID().slice(0, 8).toUpperCase()}`
    ...
    <div className="min-h-screen bg-brand-dark py-10 px-4">
      <div className="max-w-xl mx-auto">
        ...
        <button
          ...
          className="flex items-center gap-2 text-brand-muted text-sm hover:text-brand-white mb-8 transition-colors"
        >
        ...
        <div className="mb-8 text-center">
          <h1 className="font-display text-4xl text-brand-white tracking-wider mb-1">CHECKOUT</h1>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              ...
              <button
                type="submit"
                className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-brand-green text-brand-dark font-bold text-sm hover:bg-brand-green-dark transition-all active:scale-95"
              >
            ...
            >
              {orderType === 'delivery' ? (
                ...
                <div className="bg-brand-card border border-brand-border rounded-2xl p-5">
                  ...
                  <p className="text-brand-white font-semibold">{BRAND.address}</p>
            ...
            >
              ...
              <div className="bg-brand-card border border-brand-border rounded-2xl p-5 space-y-3">
                ...
                <div className="border-t border-brand-border pt-2 space-y-1">
                  ...
                  <div className="flex justify-between font-bold text-brand-white">
                    <span>Total</span>
